...
class ImbalanceCIFAR10C(CIFAR10C):
    ...
    def __init__(self, name, imb_type='exp', imb_factor=1.0, rand_number=0, train=True,
...
class CIFAR10C_brightness(ABDataset):
    def create_dataset(self, root_dir: str, split: str, transform: Optional[Compose],
                       classes: List[str], ignore_classes: List[str], idx_map: Optional[Dict[int, int]]):
        if transform is None:
            transform = cifar_like_image_train_aug() if split == 'train' else cifar_like_image_test_aug()
            self.transform = transform
        dataset = ImbalanceCIFAR10C(
            name='brightness', imb_factor=0.01, transform=transform
        )
        self.dataset1 = dataset
...
class CIFAR10C_fog(ABDataset):
    def create_dataset(self, root_dir: str, split: str, transform: Optional[Compose],
        ...
        if transform is None:
            ...
            self.transform = transform
        dataset = ImbalanceCIFAR10C(
            name='fog', imb_factor=0.01, transform=transform
        )
        self.dataset1 = dataset
...
class CIFAR10C_frost(ABDataset):
    def create_dataset(self, root_dir: str, split: str, transform: Optional[Compose],
                       classes: List[str], ignore_classes: List[str], idx_map: Optional[Dict[int, int]]):
        if transform is None:
            transform = cifar_like_image_train_aug() if split == 'train' else cifar_like_image_test_aug()
            self.transform = transform
        dataset = ImbalanceCIFAR10C(
            name='frost', imb_factor=0.01, transform=transform
        )
        self.dataset1 = dataset
...
class CIFAR10C_glassblur(ABDataset):
    def create_dataset(self, root_dir: str, split: str, transform: Optional[Compose],
                       classes: List[str], ignore_classes: List[str], idx_map: Optional[Dict[int, int]]):
        if transform is None:
            transform = cifar_like_image_train_aug() if split == 'train' else cifar_like_image_test_aug()
            self.transform = transform
        dataset = ImbalanceCIFAR10C(
            name='glass_blur', imb_factor=0.01, transform=transform
        )
        self.dataset1 = dataset
...
class CIFAR10C_pixelate(ABDataset):
    def create_dataset(self, root_dir: str, split: str, transform: Optional[Compose],
                       classes: List[str], ignore_classes: List[str], idx_map: Optional[Dict[int, int]]):
        if transform is None:
            transform = cifar_like_image_train_aug() if split == 'train' else cifar_like_image_test_aug()
            self.transform = transform
        dataset = ImbalanceCIFAR10C(
            name='pixelate', imb_factor=0.01, transform=transform
        )
        self.dataset1 = dataset
...
class CIFAR10C_impulsenoise(ABDataset):
        def create_dataset(self, root_dir: str, split: str, transform: Optional[Compose],
                           classes: List[str], ignore_classes: List[str], idx_map: Optional[Dict[int, int]]):
            if transform is None:
                transform = cifar_like_image_train_aug() if split == 'train' else cifar_like_image_test_aug()
                self.transform = transform
            dataset = ImbalanceCIFAR10C(
                name='impulse_noise', imb_factor=0.01, transform=transform
            )
            self.dataset1 = dataset
