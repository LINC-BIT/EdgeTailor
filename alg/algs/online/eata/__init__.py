...
from .util import check_model, collect_params, configure_model, EATA as RawEATA
...
class EATA(ABOnlineDAAlgorithm):
    ...
    def __init__(self, models, alg_models_manager: ABAlgModelsManager, hparams: Dict, device, random_seed, res_save_dir):
        ...
        self.eata_model = RawEATA(model, self.optimizer, None, self.hparams['fisher_alpha'], 
                             e_margin=self.hparams['e_margin'], 
                             d_margin=self.hparams['d_margin'])
    ...
    def adapt_to_target_domain(self, scenario: Scenario, exp_tracker: OnlineDATracker):
        ...
        configure_model(self.eata_model.model)
        #print(torch.load(self.hparams['fisher_file_path'])['CIFAR10'])
        fishers = torch.load(self.hparams['fisher_file_path'])[scenario.get_source_datasets_name()[0]]
        self.eata_model.fishers = fishers

        for iter_index in range(self.hparams['num_iters']):
            x, = torch.tensor([item.cpu().detach().numpy() for item in next(target_train_loader)[0:1]]).cuda()
            x = x.to(self.device)

            configure_model(self.eata_model.model)
            _, entropy_loss, ewc_loss = self.eata_model(x)
            exp_tracker.add_losses(dict(entropy=entropy_loss, ewc=ewc_loss), iter_index)
            exp_tracker.in_each_iteration_of_each_da()
            
        self.alg_models_manager.set_model(self.models, 'main model', self.eata_model.model)
