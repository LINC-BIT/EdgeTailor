...
def train(models, alg_models_manager: ABAlgModelsManager, args, exp_tracker: OnlineDATracker, scenario: Scenario, device):
    ...
    source_dataset, target_dataset = scenario.get_merged_source_dataset('train'), \
        scenario.get_limited_target_train_dataset()
    ...
    #     batch_size=args['batch_size'], shuffle=True, num_workers=0)
    source_loader = iter(scenario.build_dataloader(source_dataset, args['batch_size'], args['num_workers'], True, None))
    target_loader = iter(scenario.build_dataloader(target_dataset, args['batch_size'], args['num_workers'], True, None))
    
    # model = models.Net(task=args.task).cuda()
    ft = alg_models_manager.get_model(models, 'feature extractor')
    cls = alg_models_manager.get_model(models, 'classifier')
    model = nn.Sequential(ft, cls)
    ...
    for iter_index in tqdm(range(args['num_iters']), dynamic_ncols=True, leave=False):
        # adjust_learning_rate(optimizer, epoch, batch_idx, len(source_loader))
        p = iter_index / args['num_iters']
        constant = 2. / (1. + np.exp(-10 * p)) - 1
        
        batch_s = next(source_loader)
        batch_t = next(target_loader)

        data_s, target_s = batch_s[0:2]

        data_t  =  torch.tensor([item.cpu().detach().numpy() for item in batch_t[0:1]]).cuda()
        ...
        output_t = cls(grad_reverse(ft(data_t), constant))

        loss_cel = criterion_cel(output_s, target_s)

        output_t_prob_unk = F.softmax(output_t, dim=1)[:,-1]
        loss_adv = criterion_bce(output_t_prob_unk, torch.tensor([args['th']] * args['batch_size']).cuda())
