...
#     'vgg16': VGG16Fc
# }
from benchmark.exp.alg_model_manager import longlabel,iflong
...
def EntropyLoss(predict_prob, class_level_weight=None, instance_level_weight=None, epsilon=1e-6):
    ...
    entropy = -predict_prob*torch.log(predict_prob + epsilon)#*(1-predict_prob + epsilon)**2
...
def train(models, hparams, device, alg_models_manager: ABAlgModelsManager, exp_tracker: OnlineDATracker, scenario: Scenario):
    ...
    for iter_index in tqdm(range(hparams['num_iters']), dynamic_ncols=True, leave=False):
        ...
        im_source, label_source = next(source_loader)[0:2]
        im_target, =  torch.tensor([item.cpu().detach().numpy() for item in next(target_loader)[0:1]]).cuda()
        ...
        from benchmark.exp.alg_model_manager import  FocalLoss111,LabelSmoothSoftmaxCEV1,FocalLoss11,BCEFocalLoss,create_loss

        #ce = nn.CrossEntropyLoss(reduction='none')(feature_source, label_source.long()).view(-1, 1)
        ce = FocalLoss111()(feature_source, label_source.long()).view(-1, 1)
        ...
        #tmp = weight * BCEFocalLoss()(domain_prob_discriminator_source,torch.ones_like(domain_prob_discriminator_source))
        #print('tmp:',tmp)
        ...
        #adv_loss += BCEFocalLoss()(domain_prob_discriminator_target, torch.zeros_like(domain_prob_discriminator_target))

        def smooth(label):
            num_classes = feature_source.size(1)
            ...
            lb_pos, lb_neg = 1. - 0.1, 0.1 / num_classes
            lb_one_hot = torch.empty_like(feature_source).fill_(
        ...
        def one_hot(label):
            ...
            return torch.clamp(F.one_hot(label.to(torch.int64), num_classes=feature_source.size(1)).float(), 1., 1. - 1e-8)
        
        if torch.isnan(predict_prob_source_aug).sum() > 0 or torch.isnan(predict_prob_target_aug).sum() > 0:
            print('loss exploded!')
            return

        longlab=longlabel.expand(predict_prob_source_aug.size())
        ...
        # try:
        #print('smooth',torch.clamp(predict_prob_source_aug, 1e-8, 1. - 1e-8))

        #ce_aug = nn.BCELoss(reduction='none')(torch.clamp(predict_prob_source_aug, 1e-8, 1. - 1e-8),one_hot(label_source))
        ce_aug = nn.BCELoss(reduction='none')(torch.clamp(predict_prob_source_aug, 1e-8, 1. - 1e-8),longlab)
        #print('label_source',label_source)

        #ce_aug = BCEFocalLoss()(torch.clamp(predict_prob_source_aug, 1e-8, 1. - 1e-8),smooth(label_source))
        ce_aug = torch.sum(ce_aug) / label_source.numel()


        iflonglab=torch.zeros_like(domain_prob_source_aug)
        iflonglab=iflong(iflonglab,label_source)
        #print('label_source',label_source)
        #print('iflonglab',iflonglab)

        #adv_loss_aug = nn.BCELoss()(torch.clamp(domain_prob_source_aug, 1e-8, 1. - 1e-8), torch.ones_like(domain_prob_source_aug))
        #print('domain_prob_source_aug',domain_prob_source_aug)
        #print('predict_prob_source_aug', predict_prob_source_aug.shape)
        #print('xx', xx)

        #adv_loss_aug += nn.BCELoss()(torch.clamp(domain_prob_target_aug, 1e-8, 1. - 1e-8), torch.zeros_like(domain_prob_target_aug))
        adv_loss_aug =2* nn.BCELoss()(torch.clamp(domain_prob_target_aug, 1e-8, 1. - 1e-8),iflonglab)

        #adv_loss_aug = BCEFocalLoss()(torch.clamp(domain_prob_source_aug, 1e-8, 1. - 1e-8), torch.ones_like(domain_prob_source_aug))
        #adv_loss_aug += BCEFocalLoss()(torch.clamp(domain_prob_target_aug, 1e-8, 1. - 1e-8), torch.zeros_like(domain_prob_target_aug))
