...
def train(models, alg_models_manager: ABAlgModelsManager, config, exp_tracker: OfflineTrainTracker, scenario: Scenario, device):
    ...
    source_loader: Iterator[Any] = iter(scenario.build_dataloader(source_dataset, config['batch_size'], config['num_workers'],
                                              True, None))
    ...
    for iter_index in tqdm(range(config['num_iters']), dynamic_ncols=True):
        ...
        #predict_prob_source=predict_prob_source[:, 1:]
        predict_prob_source = predict_prob_source
