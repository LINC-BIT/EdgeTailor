...
class NormalSourceTrain(ABOfflineTrainAlgorithm):
    ...
    def train(self, scenario: Scenario, exp_tracker: OfflineTrainTracker):
        ...
        if self.hparams['Resampling'] == 1:
            ...
            balanced_trainset = {n: square_root_biased_sampling(d.dataset, self.hparams['oversample_factor'], self.hparams['undersample_factor']) for n, d in train_sets.items()}
            ...
            resampled_class_counts = {n: np.bincount(np.array(d.dataset.targets)[balanced_trainset[n].indices]) for n, d in
             train_sets.items()}
        ...
        if self.hparams['Resampling'] == 2:
            ...
            balanced_trainset={n: progressively_balanced_sampling(d.dataset, class_weights[n])[0]  for n, d in train_sets.items()}
        ...
        if (self.hparams['Augmentation'] == 1 or self.hparams['Augmentation'] == 3 or self.hparams['Augmentation'] == 4):
            cls_num_list={n: d.dataset.get_cls_num_list()  for n, d in train_sets.items()}
            #weighted_sampler = train_sets.get_weighted_sampler()
            weighted_sampler ={n: d.dataset.get_weighted_sampler(weighted_alpha=self.hparams['weighted_alpha'])  for n, d in train_sets.items()}
        ...
        if self.hparams['Augmentation'] == 2 :
            cls_num_list = {n: d.dataset.get_cls_num_list() for n, d in train_sets.items()}
            remix_trainset = {n: RemixDataset(d.dataset, cls_num_list[n], beta=1.0,tau=self.hparams['tau'], kappa=self.hparams['kappa'])  for n, d in train_sets.items()}
        ...
        for iter_index in tqdm.tqdm(range(self.hparams['num_iters']), desc='iterations',
            ...
            for train_loader_name, train_loader in train_loaders.items():
                ...
                if (self.hparams['Augmentation'] == 1 or self.hparams['Augmentation'] == 3) and (self.hparams['mixup_prob']>mix_prob) :
                    try:
                ...
                #
                # # 假设 x 是模型输入，y 是目标标签
                # print(f"x is CUDA tensor: {x.is_cuda}")
                ...
                # else:
                #     print("No NaN or Inf in input x.")
                #
                # # 将张量移动到 CPU 进行检查
                # x_cpu = x.cpu()
                # print(f"x on CPU contains NaN: {torch.isnan(x_cpu).any()}")
                #
                # # 检查目标标签的范围
                # print(f"y min: {y.min()}, y max: {y.max()}")
                # if y.min() < 0 or y.max() >= 200:
                #     print("y values are out of range!")
                #     y = y.clamp(0, 200 - 1)

                # print(f"Model output shape: {x.shape}")
                # n_classes = x.shape[1]
                # print(f"Number of classes (n_classes): {n_classes}")
                # print(f"Target shape: {y.shape}")  # e.g., [batch_size] or [batch_size, height, width]
                # print(f"Target min: {y.min().item()}, max: {y.max().item()}")
                task_loss.backward()
                optimizer.step()
